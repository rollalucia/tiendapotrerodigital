...
<nav class="py-2 bg-light border-bottom">
  ...
  <header class="mb-4 border-bottom">
    <div class="container d-flex flex-wrap justify-content-center align-items-center">
      <a href="#" class="d-flex align-items-center mb-0 mb-lg-0 me-lg-auto text-dark text-decoration-none">
  ...
  <div class="album py-5 bg-light">
    ...
    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
        <?php 
        if ($user=="potrero" && $pass=="1234"){
          echo '<div class="col">';
          echo '<form class="card shadow-sm" method="POST" action="agregar.php">';
          // echo '<img class="img-responsive w-100 alto-minimo" src="#" alt="" width="100%">';
            echo '<div class="card-body">';
              ...
              echo '<div class="form-floating mb-1 fs-6 lh-1">';
                echo '<input type="text" class="form-control" id="prenda" placeholder="Tipo de prenda" name="prenda" autofocus>';
                echo '<label for="prenda">Tipo de prenda</label>';
              echo '</div>';
              echo '<div class="form-floating mb-1 fs-6 lh-1">';
                echo '<input type="text" class="form-control" id="marca" placeholder="Marca" name="marca">';
                echo '<label for="marca">Marca</label>';
              echo '</div>';
              echo '<div class="form-floating mb-1 fs-6 lh-1">';
                echo '<input type="text" class="form-control" id="talle" placeholder="Talle" name="talle">';
                echo '<label for="talle">Talle</label>';
              echo '</div>';
              echo '<div class="form-floating mb-1 fs-6 lh-1">';
                echo '<input type="text" class="form-control" id="color" placeholder="Color" name="color">';
        ...
        echo '<div class="card shadow-sm">';
          if ($user=="potrero" && $pass=="1234"){
            echo '<div class="btn-group d-flex justify-content-end posicion">';
                  print '<a class="edit text-primary fs-5" href="editar.php?id='.$fila['id'].'"><i class="fa-solid fa-pen-to-square"></i></a>';
                  print '<a class="borrar text-danger fs-5" href="borrar.php?id='.$fila['id'].'"><i class="fa-solid fa-circle-minus"></i></a>';
            echo '</div>';
          } else {
            echo '';
          }              
            echo '<img class="img-responsive w-100 alto-minimo" src="data:image/png; base64,'. base64_encode($fila['imagen']).'" alt="" width="100%">';
            echo '<div class="card-body">';
              echo '<h5 class="card-title text-uppercase">'.$fila["tipo_de_prenda"]." ". $fila["marca"].'</h5>';
              echo '<p class="card-text">Talle '.strtoupper($fila["talle"]).'. Color '.$fila["color"].'.</p>';
              echo '<div class="d-flex justify-content-between">';
                echo '<p class="fs-2 fw-bold my-0">$'.$fila["precio"].'</p>';
              echo '</div>';
            echo '</div>';
          echo '</div>';
        echo '</div>';
